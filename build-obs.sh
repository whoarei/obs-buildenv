...
if [ ! -f "$BUILD_DIR/CMakeCache.txt" ]; then
  ...
  cmake -S "$OBS_SRC_DIR" -B "$BUILD_DIR" -G Ninja \
    ...
    -DCPACK_DEBIAN_PACKAGE_DEPENDS='mesa25-rk3588-local (>= 25.0.7-2~ans1), qt6.2-gles-local (>= 6.2.4-1~ans1), ffmpeg6.1-ans-local (>= 6.1.6-1~ans1), rockchip-mpp-local (>= 1.3.9-1~ans1)' \
